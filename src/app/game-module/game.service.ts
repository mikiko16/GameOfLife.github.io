...
export class GameOfLifeService {
   ...
   */
  initialize(rows = APP_CONSTANTS.initializeRows, cols = APP_CONSTANTS.initializeCols) {
    const minDimension = APP_CONSTANTS.minDimension;
   ...
   */
  randomizeCellStates(percentAlive = APP_CONSTANTS.percentAlive) {
    if (!this.cells) {
  ...
  playAudio(){
      let audio = new Audio();
      audio.src = "../../assets/music/ForestWalk.mp3";
      audio.load();
      audio.play();
  }
